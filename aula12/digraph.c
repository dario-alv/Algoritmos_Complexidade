 ...
 Autor : Dário César Loureiro Alves    Mec : 67591
 Autor : Mariana Pinheiro Gomes        Mec : 73211
...
{ 
	...
	if(((PtVertex) Vertex->PtElem)->InDeg == 0) {
		if(((PtVertex) Vertex->PtElem)->OutDeg == 0) return DISC;
		return SOURCE; }
	if(((PtVertex) Vertex->PtElem)->OutDeg == 0) return SINK;
	return OK;
}
...
{ 
	...
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	if((Vertex = OutPosition(pdig->Head, pv)) == NULL) return NO_VERTEX;
	*pcent = (double) ((PtVertex) Vertex->PtElem)->OutDeg / (pdig->NVertexes - 1);
	return OK;
}
...
{ 
	...
	if(pdig == NULL) return NO_DIGRAPH;
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	PtBiNode Vertex = pdig->Head, Max = pdig->Head; 
	unsigned int verAdj = 0;
	while(Vertex != NULL) {
		if(((PtVertex) Vertex->PtElem)->OutDeg > verAdj) {
			verAdj = ((PtVertex) Vertex->PtElem)->OutDeg;
			Max = Vertex;
		}
		Vertex = Vertex->PtNext;
	}
	*pv = Max->Number;
	*pmax = (double) verAdj / (pdig->NVertexes - 1);
	return OK;
}
...
{ 
	...
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	if((Vertex = OutPosition(pdig->Head, pv)) == NULL) return NO_VERTEX;
	if(Vertex->PtAdj == NULL) return NO_EDGE;
	Edge = Vertex->PtAdj; unsigned int sucDegree = 0;
	while(Edge != NULL) {
		sucDegree = sucDegree + ((PtVertex) Edge->PtAdj->PtElem)->OutDeg;
		Edge = Edge->PtNext;
	}
	*pmsuc = (double) sucDegree / ((PtVertex) Vertex->PtElem)->OutDeg;
	return OK;
}
...
{ 
	...
	while(Vertex != NULL) {
		if( ((PtVertex) Vertex->PtElem)->OutDeg == 0 && ((PtVertex) Vertex->PtElem)->InDeg == 0) {
			if(QueueEnqueue (Queue, &Vertex->Number) == NO_MEM) {
				QueueDestroy(&Queue);
				*pqueue = NULL;
				return NO_MEM;
			}
...
{ 
	PtQueue Queue; PtBiNode Vertex, Temp, Edge;
	if(pdig == NULL) return NO_DIGRAPH;
	if(pdig->NVertexes == 0) return DIGRAPH_EMPTY;
	if(pqueue == NULL) return NULL_PTR;
	if ((Queue = QueueCreate (sizeof (int))) == NULL) {
	...
	if((Vertex = OutPosition(pdig->Head, pv)) == NULL) return NO_VERTEX;
	for(Temp = pdig->Head; Temp != NULL; Temp = Temp->PtNext) {
		for(Edge = Temp->PtAdj; Edge != NULL; Edge = Edge->PtNext) {
			if(Edge->PtAdj == Vertex) {
				if(QueueEnqueue (Queue, &Temp->Number) == NO_MEM) {
					QueueDestroy(&Queue);
					*pqueue = NULL;
					return NO_MEM;
				}
...
{ 
	...
	// como pedido no guião, considerarmos  que os vértices NÃO! se encontram sequencialmente numerados
	int findEdge;
	for(Vertex = pdig->Head; Vertex != NULL; Vertex = Vertex->PtNext) {
		for(Temp = pdig->Head; Temp != NULL; Temp = Temp->PtNext) {
			if(Temp == Vertex) continue;
			findEdge = 0;
			for(Edge = Vertex->PtAdj; Edge != NULL; Edge = Edge->PtNext) {
				if(Edge->PtAdj == Temp) {
					findEdge = 1;
				}
			}
			if(!findEdge) {
				if(QueueEnqueue (Queue, &Vertex->Number) == NO_MEM || QueueEnqueue (Queue, &Temp->Number) == NO_MEM) {
					QueueDestroy(&Queue);
					*pqueue = NULL;
					return NO_MEM;
				}
	...
	/* Algoritmo que assume vertices ordenados de forma sequencial, tb funcional 
	
	for(Vertex = pdig->Head; Vertex != NULL; Vertex = Vertex->PtNext) {
		Edge = Vertex->PtAdj;
		for(Temp = pdig->Head; Temp != NULL; Temp = Temp->PtNext) {
			if(Temp == Vertex) continue;
			if(Edge == NULL || Edge->PtAdj != Temp) {
				if(QueueEnqueue (Queue, &Vertex->Number) == NO_MEM || QueueEnqueue (Queue, &Temp->Number) == NO_MEM) {
					QueueDestroy(&Queue);
					*pqueue = NULL;
					return NO_MEM;
