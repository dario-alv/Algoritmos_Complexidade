/*******************************************************************************
 ...
 Nome : Dário César Loureiro Alves                    NMEC : 67591
...
void ABPDeleteRep (PtABPNode *proot, int pelem)	/* remoção repetitiva - repetitive deletion */
{
	...
	if (*proot == NULL) { Error = NO_ELEM; return; }	/* arvore vazia - empty tree */
...
/* função repetitiva - insira o seu codigo */
	Error = ABP_EMPTY; //preservar o erro caso não exista valor
	if(proot == NULL) {return NULL;}
	PtABPNode Floor = NULL;
	while (proot != NULL) {
		if(proot->Elem < pvalue) {
			if(Floor == NULL || proot->Elem > Floor->Elem) {
			Error = OK;
			Floor = proot;
			}
			proot = proot->PtRight;
...
/* função recursiva - insira o seu codigo */ 
	...
	if(proot == NULL) {return NULL;}
	PtABPNode Temp = NULL;
	if(proot->Elem < pvalue) Temp = ABPCeilValue(proot->PtRight, pvalue);
	else if(proot->Elem > pvalue) {
		Temp = ABPCeilValue(proot->PtLeft, pvalue);
		if(Temp == NULL) {Error = OK; return proot;}
	}
	else {Error = OK; return proot;}
	return Temp;
}
...
/* função recursiva - insira o seu codigo */ 
	if(proot == NULL) {Error = ABP_EMPTY; return 0;}
	int val;
	if(proot->Elem<pvalue) val = 1 + ABPSize(proot->PtLeft) + ABPRank(proot->PtRight, pvalue);
	else val = ABPRank(proot->PtLeft, pvalue);
	Error = OK;
	return val;
}
...
/* função repetitiva - insira o seu codigo */ 
	...
	StackPush (Stack, &Node);	/* armazenar a raiz - storing the root */
	int par;
	if(ABPMinRec(proot)->Elem % 2 == 0) par = 1;
	else par = 0;
	while (!StackIsEmpty (Stack))
	{
		if (Node != NULL)	/* não é um no externo - not an external node */
		...
		else	/* é um no externo */
		{
			...
			if (!StackIsEmpty (Stack))
			{
				StackPop (Stack, &Node);	/* recuperar o no anterior - retrieve and preview node */
				if(par) {
					par = 0; /* pŕoximo nº tem que ser impar */
					if(Node->Elem % 2 != 0) {StackDestroy (&Stack); return 0;}
				} else {
					par = 1;
					if(Node->Elem % 2 == 0) {StackDestroy (&Stack); return 0;}
				}
